"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.registerSlashCommands = registerSlashCommands;var _nodeCrypto = await jitiImport("node:crypto");
...
var _slashLiveState = await jitiImport("./slash-live-state.ts");
...
const makeAgentCompletions = (state, multiAgent) => (prefix) => {
  const agents = (0, _agents.discoverAgents)(state.baseCwd, "both").agents;
  if (!multiAgent) {
...
const discoverSavedChains = (cwd) => {
  ...
  for (const chain of (0, _agents.discoverAgentsAll)(cwd).chains) {
...
const mapSavedChainSteps = (chain, worktree = false) => {
  return chain.steps.map((step) => {
    if ((0, _settings.isParallelStep)(step)) return worktree ? { ...step, worktree: true } : { ...step };
    return {
      agent: step.agent,
...
params)
{
  return new Promise((resolve, reject) => {
    ...
    const onUpdate = (data) => {
      ...
      (0, _slashLiveState.applySlashUpdate)(requestId, update);
      if (!ctx.hasUI) return;
    ...
    ctx.ui.onTerminalInput((input) => {
      if (!(0, _piTui.matchesKey)(input, _piTui.Key.escape)) return undefined;
      pi.events.emit(_types.SLASH_SUBAGENT_CANCEL_EVENT, { requestId });
      finish(() => reject(new Error("Cancelled")));
      return { consume: true };
    }) :
    undefined;

    const unsubStarted = pi.events.on(_types.SLASH_SUBAGENT_STARTED_EVENT, onStarted);
    const unsubResponse = pi.events.on(_types.SLASH_SUBAGENT_RESPONSE_EVENT, onResponse);
    const unsubUpdate = pi.events.on(_types.SLASH_SUBAGENT_UPDATE_EVENT, onUpdate);
    ...
    pi.events.emit(_types.SLASH_SUBAGENT_REQUEST_EVENT, { requestId, params });
...
params)
{
  ...
  const requestId = (0, _nodeCrypto.randomUUID)();
  const initialDetails = (0, _slashLiveState.buildSlashInitialResult)(requestId, params);
  const initialText = extractSlashMessageText(initialDetails.result.content) || "Running subagent...";
  pi.sendMessage({
    customType: _types.SLASH_RESULT_TYPE,
    content: initialText,
    display: true,
  ...
  try {
    ...
    const finalDetails = (0, _slashLiveState.finalizeSlashResult)(response);
    pi.sendMessage({
      customType: _types.SLASH_RESULT_TYPE,
    ...
    const failedDetails = (0, _slashLiveState.failSlashResult)(requestId, params, message);
    pi.sendMessage({
      customType: _types.SLASH_RESULT_TYPE,
...
state)
{
  ...
  pi.registerCommand("chain", {
    ...
    handler: async (args, ctx) => {
      ...
      const params = { chain, task: parsed.task, clarify: false, agentScope: "both" };
      if (bg) params.async = true;
      if (fork) params.context = "fork";
      await runSlashSubagent(pi, ctx, params);
    }
