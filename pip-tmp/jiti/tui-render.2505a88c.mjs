"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.buildWidgetLines = buildWidgetLines;exports.renderSubagentResult = renderSubagentResult;exports.renderWidget = renderWidget;exports.stopResultAnimations = stopResultAnimations;exports.stopWidgetAnimation = stopWidgetAnimation;exports.syncResultAnimation = syncResultAnimation;



var path = _interopRequireWildcard(await jitiImport("node:path"));

var _piCodingAgent = await jitiImport("@mariozechner/pi-coding-agent");
...
var _parallelGroups = await jitiImport("../runs/background/parallel-groups.ts");
var _statusFormat = await jitiImport("../shared/status-format.ts");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);} /**
...
function truncLine(text, maxWidth) {
  if ((0, _piTui.visibleWidth)(text) <= maxWidth) return text;
  ...
  while (i < text.length) {
    ...
    for (const seg of segmenter.segment(textPortion)) {
      ...
      const graphemeWidth = (0, _piTui.visibleWidth)(grapheme);
...
function resultIsRunning(result) {
  return result.details?.progress?.some((entry) => entry.status === "running") ||
  result.details?.results.some((entry) => entry.progress?.status === "running") ||
  false;
}

function stopResultAnimation(context) {
...
function syncResultAnimation(result, context) {
...
{
  if (result.messages) {
    return (0, _utils.getDisplayItems)(result.messages).
    filter((item) => item.type === "tool").
    map((item) => (0, _formatters.formatToolCall)(item.name, item.args, expanded));
  }
  return result.toolCalls?.map((toolCall) => expanded ? toolCall.expandedText : toolCall.text) ?? [];
}


function formatCurrentToolLine(progress, availableWidth, expanded) {
  ...
  ` | ${(0, _formatters.formatDuration)(Math.max(0, Date.now() - progress.currentToolStartedAt))}` :
  "";
...
function buildLiveStatusLine(progress) {
  return (0, _statusFormat.formatActivityLabel)(progress.lastActivityAt, progress.activityState);
}
...
function formatTokenStat(tokens) {
  return `${(0, _formatters.formatTokens)(tokens)} token`;
}
...
function formatProgressStats(theme, progress, includeDuration = true) {
  ...
  if (includeDuration && progress.durationMs > 0) parts.push((0, _formatters.formatDuration)(progress.durationMs));
...
function resultGlyph(result, output, theme, running = result.progress?.status === "running") {
...
function hasAnimatedWidgetJobs(jobs) {
...
function widgetActivity(job) {
  const facts = [];
  if (job.currentTool && job.currentToolStartedAt !== undefined) facts.push(`${job.currentTool} ${(0, _formatters.formatDuration)(Math.max(0, Date.now() - job.currentToolStartedAt))}`);else
  if (job.currentTool) facts.push(job.currentTool);
  if (job.currentPath) facts.push((0, _formatters.shortenPath)(job.currentPath));
  if (job.turnCount !== undefined) facts.push(`${job.turnCount} turns`);
  if (job.toolCount !== undefined) facts.push(`${job.toolCount} tools`);
  const activity = (0, _statusFormat.formatActivityLabel)(job.lastActivityAt, job.activityState) ?? (
...
function widgetStepGlyph(status, theme) {
...
function widgetStepActivity(step) {
  const facts = [];
  if (step.currentTool && step.currentToolStartedAt !== undefined) facts.push(`${step.currentTool} ${(0, _formatters.formatDuration)(Math.max(0, Date.now() - step.currentToolStartedAt))}`);else
  if (step.currentTool) facts.push(step.currentTool);
  if (step.currentPath) facts.push((0, _formatters.shortenPath)(step.currentPath));
  ...
  const activity = (0, _statusFormat.formatActivityLabel)(step.lastActivityAt, step.activityState);
  if (activity && facts.length) return `${activity} · ${facts.join(" · ")}`;
  if (activity) return activity;
  return facts.join(" · ");
}
...
function widgetChainDetails(job, theme, expanded = false, width = getTermWidth()) {
  ...
  for (const span of buildAsyncChainStepSpans(total, job.steps.length, job.parallelGroups)) {
    ...
    if (span.isParallel) {
      const status = (0, _statusFormat.aggregateStepStatus)(steps);
      lines.push(`  ${widgetStepGlyph(status, theme)} Step ${span.stepIndex + 1}/${total}: ${themeBold(theme, "parallel group")} ${theme.fg("dim", "·")} ${theme.fg("dim", (0, _statusFormat.formatParallelOutcome)(steps, span.count))}`);
      continue;
    }
    const step = steps[0];
    if (!step) {
...
function widgetParallelAgentDetails(job, theme) {
  if (!job.steps?.length) return [];
...
function buildChainStepSpans(chainAgents) {
...
function buildMultiProgressLabel(details, hasRunning) {
  ...
  if (details.mode === "parallel") {
    ...
    `${(0, _statusFormat.formatAgentRunningLabel)(running)} · ${done}/${totalCount} done` :
  ...
  if (activeParallelGroup) {
    ...
    const headerLabel = hasRunning ?
    `step ${currentStepIndex + 1}/${totalSteps} · parallel group: ${(0, _statusFormat.formatAgentRunningLabel)(running)} · ${done}/${groupSize} done` :
...
function widgetStats(job, theme) {
  ...
  if (job.activeParallelGroup) {
    ...
    if (job.mode === "parallel") {
      if (job.status === "running" && running > 0) parts.push((0, _statusFormat.formatAgentRunningLabel)(running));
      ...
      if (job.status === "running" && running > 0) groupParts.unshift((0, _statusFormat.formatAgentRunningLabel)(running));
    ...
    if (job.mode === "chain" && job.parallelGroups?.length) {
      const total = job.chainStepCount ?? stepsTotal;
      parts.push(`step ${(0, _parallelGroups.flatToLogicalStepIndex)(job.currentStep, total, job.parallelGroups) + 1}/${total}`);
    } else {
  ...
  if (job.startedAt) parts.push((0, _formatters.formatDuration)(Math.max(0, endTime - job.startedAt)));
  return statJoin(theme, parts);
}
...
function widgetStepStats(theme, step) {
  ...
  step.durationMs !== undefined ? (0, _formatters.formatDuration)(step.durationMs) : ""]
  );
}

function widgetStepActivityLine(step, width, expanded) {
...
width)
{
  ...
  if (step.status === "running") {
    ...
    if (output) lines.push(`    ${theme.fg("dim", `output: ${(0, _formatters.shortenPath)(output)}`)}`);
    if (expanded) {
...
function buildWidgetComponent(jobs, expanded) {
  return (_tui, theme) => {
    ...
    for (const line of fitWidgetLineBudget(lines, theme, width, expanded)) container.addChild(new _piTui.Text(line, 1, 0));
    return container;
  };
}

function buildWidgetLines(jobs, theme, width = getTermWidth(), expanded = false) {
  ...
  let slots = _types.MAX_WIDGET_JOBS;
...
function refreshAnimatedWidget() {
  try {
    if (!latestWidgetCtx?.hasUI || latestWidgetJobs.length === 0) return;
    latestWidgetCtx.ui.setWidget(_types.WIDGET_KEY, buildWidgetComponent(latestWidgetJobs, latestWidgetCtx.ui.getToolsExpanded?.() ?? false));
...
function ensureWidgetAnimation() {
...
function renderWidget(ctx, jobs) {
  if (jobs.length === 0) {
    stopWidgetAnimation();
    if (ctx.hasUI) ctx.ui.setWidget(_types.WIDGET_KEY, undefined);
    return;
  }
  if (!ctx.hasUI) {
    stopWidgetAnimation();
  ...
  ctx.ui.setWidget(_types.WIDGET_KEY, buildWidgetComponent(jobs, ctx.ui.getToolsExpanded?.() ?? false));
  if (hasAnimatedWidgetJobs(jobs)) ensureWidgetAnimation();else
  stopWidgetAnimation();
}

function renderSingleCompact(d, r, theme) {
  const output = r.truncation?.text || (0, _utils.getSingleResultOutput)(r);
  ...
  const c = new _piTui.Container();
  const width = getTermWidth() - 4;
  c.addChild(new _piTui.Text(truncLine(`${resultGlyph(r, output, theme, isRunning)} ${theme.fg("toolTitle", theme.bold(r.agent))}${contextBadge}${stats ? ` ${theme.fg("dim", "·")} ${stats}` : ""}`, width), 0, 0));

  if (isRunning && r.progress) {
    const activity = compactCurrentActivity(r.progress);
    c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `  ⎿  ${activity}`), width), 0, 0));
    const liveStatus = buildLiveStatusLine(r.progress);
    if (liveStatus && liveStatus !== activity) c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `     ${liveStatus}`), width), 0, 0));
    c.addChild(new _piTui.Text(truncLine(theme.fg("accent", "  Press Ctrl+O for live detail"), width), 0, 0));
    if (r.artifactPaths) c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `  output: ${(0, _formatters.shortenPath)(r.artifactPaths.outputPath)}`), width), 0, 0));
    return c;
  }

  c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `  ⎿  ${resultStatusLine(r, output)}`), width), 0, 0));
  const preview = firstOutputLine(output);
  if (preview && r.exitCode === 0 && !hasEmptyTextOutputWithoutOutputTarget(r.task, output)) {
    c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `     ${preview}`), width), 0, 0));
  }
  if (r.sessionFile) c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `  session: ${(0, _formatters.shortenPath)(r.sessionFile)}`), width), 0, 0));
  if (r.artifactPaths) c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `  output: ${(0, _formatters.shortenPath)(r.artifactPaths.outputPath)}`), width), 0, 0));
  if (r.truncation?.artifactPath) c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `  full output: ${(0, _formatters.shortenPath)(r.truncation.artifactPath)}`), width), 0, 0));
  return c;
}

function renderMultiCompact(d, theme) {
  ...
  const c = new _piTui.Container();
  const width = getTermWidth() - 4;
  c.addChild(new _piTui.Text(truncLine(`${glyph} ${theme.fg("toolTitle", theme.bold(d.mode))}${contextBadge}${stats ? ` ${theme.fg("dim", "·")} ${stats}` : ""}`, width), 0, 0));
  ...
  for (let i = displayStart; i < displayEnd; i++) {
    ...
    if (!r) {
      c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `  ◦ ${itemTitle} ${rowNumber}: ${agentName} · pending`), width), 0, 0));
      continue;
    }
    const output = (0, _utils.getSingleResultOutput)(r);
    ...
    c.addChild(new _piTui.Text(truncLine(`  ${line}`, width), 0, 0));
    if (rRunning && rProg && "status" in rProg) {
      const activity = compactCurrentActivity(rProg);
      c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `    ⎿  ${activity}`), width), 0, 0));
      c.addChild(new _piTui.Text(truncLine(theme.fg("accent", "    Press Ctrl+O for live detail"), width), 0, 0));
    } else if (!rPending && (r.exitCode !== 0 || r.interrupted || r.detached || hasEmptyTextOutputWithoutOutputTarget(r.task, output))) {
      c.addChild(new _piTui.Text(truncLine(theme.fg(r.exitCode !== 0 ? "error" : "dim", `    ⎿  ${resultStatusLine(r, output)}`), width), 0, 0));
    }
    const outputTarget = extractOutputTarget(r.task);
    if (outputTarget) c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `    output: ${outputTarget}`), width), 0, 0));
    if (r.artifactPaths) c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `    output: ${(0, _formatters.shortenPath)(r.artifactPaths.outputPath)}`), width), 0, 0));
  }
  if (d.artifacts) c.addChild(new _piTui.Text(truncLine(theme.fg("dim", `  artifacts: ${(0, _formatters.shortenPath)(d.artifacts.dir)}`), width), 0, 0));
  return c;
}
...
theme)
{
  ...
  if (!d || !d.results.length) {
    ...
    return new _piTui.Text(truncLine(`${contextPrefix}${text}`, getTermWidth() - 4), 0, 0);
  }

  const expanded = options.expanded;
  const mdTheme = (0, _piCodingAgent.getMarkdownTheme)();
  ...
  if (d.mode === "single" && d.results.length === 1) {
    ...
    ` | ${r.progress.toolCount} tools, ${(0, _formatters.formatTokens)(r.progress.tokens)} tok, ${(0, _formatters.formatDuration)(r.progress.durationMs)}` :
    r.progressSummary ?
    ` | ${r.progressSummary.toolCount} tools, ${(0, _formatters.formatTokens)(r.progressSummary.tokens)} tok, ${(0, _formatters.formatDuration)(r.progressSummary.durationMs)}` :
    "";
    ...
    const c = new _piTui.Container();
    c.addChild(new _piTui.Text(fit(`${icon} ${theme.fg("toolTitle", theme.bold(r.agent))}${contextBadge}${progressInfo}`), 0, 0));
    c.addChild(new _piTui.Spacer(1));
    const taskMaxLen = Math.max(20, w - 8);
    const taskPreview = expanded || r.task.length <= taskMaxLen ?
    r.task :
    `${r.task.slice(0, taskMaxLen)}...`;
    c.addChild(
      new _piTui.Text(fit(theme.fg("dim", `Task: ${taskPreview}`)), 0, 0)
    );
    c.addChild(new _piTui.Spacer(1));
    ...
    if (isRunning && r.progress) {
      ...
      if (toolLine) {
        c.addChild(new _piTui.Text(fit(theme.fg("warning", `> ${toolLine}`)), 0, 0));
      }
      const liveStatusLine = buildLiveStatusLine(r.progress);
      if (liveStatusLine) {
        c.addChild(new _piTui.Text(fit(theme.fg("accent", liveStatusLine)), 0, 0));
      }
      c.addChild(new _piTui.Text(fit(theme.fg("accent", "Press Ctrl+O for live detail")), 0, 0));
      if (r.artifactPaths) {
        c.addChild(new _piTui.Text(fit(theme.fg("dim", `Artifacts: ${(0, _formatters.shortenPath)(r.artifactPaths.outputPath)}`)), 0, 0));
      }
      if (r.progress.recentTools?.length) {
        for (const t of r.progress.recentTools.slice(-3)) {
          ...
          c.addChild(new _piTui.Text(fit(theme.fg("dim", `${t.tool}: ${argsPreview}`)), 0, 0));
        }
      }
      for (const line of (r.progress.recentOutput ?? []).slice(-5)) {
        c.addChild(new _piTui.Text(fit(theme.fg("dim", `  ${line}`)), 0, 0));
      }
    ...
    if (expanded) {
      for (const line of toolCallLines) {
        c.addChild(new _piTui.Text(fit(theme.fg("muted", line)), 0, 0));
      }
      if (toolCallLines.length) c.addChild(new _piTui.Spacer(1));
    }

    if (output) c.addChild(new _piTui.Markdown(output, 0, 0, mdTheme));
    c.addChild(new _piTui.Spacer(1));
    if (r.skills?.length) {
      c.addChild(new _piTui.Text(fit(theme.fg("dim", `Skills: ${r.skills.join(", ")}`)), 0, 0));
    }
    if (r.skillsWarning) {
      c.addChild(new _piTui.Text(fit(theme.fg("warning", `Warning: ${r.skillsWarning}`)), 0, 0));
    }
    if (r.attemptedModels && r.attemptedModels.length > 1) {
      c.addChild(new _piTui.Text(fit(theme.fg("dim", `Fallbacks: ${r.attemptedModels.join(" → ")}`)), 0, 0));
    }
    c.addChild(new _piTui.Text(fit(theme.fg("dim", (0, _formatters.formatUsage)(r.usage, r.model))), 0, 0));
    if (r.sessionFile) {
      c.addChild(new _piTui.Text(fit(theme.fg("dim", `Session: ${(0, _formatters.shortenPath)(r.sessionFile)}`)), 0, 0));
    }
  ...
  hasEmptyTextOutputWithoutOutputTarget(r.task, (0, _utils.getSingleResultOutput)(r))
  );
  ...
  ` | ${totalSummary.toolCount} tools, ${(0, _formatters.formatTokens)(totalSummary.tokens)} tok, ${(0, _formatters.formatDuration)(totalSummary.durationMs)}` :
  "";
  ...
  map((agent, i) => {
    ...
    Boolean(isComplete) &&
    hasEmptyTextOutputWithoutOutputTarget(result.task, (0, _utils.getSingleResultOutput)(result));
  ...
  if (chainVis) {
    c.addChild(new _piTui.Text(fit(`  ${chainVis}`), 0, 0));
  }
  ...
  for (let i = displayStart; i < displayEnd; i++) {
    ...
    if (!r) {
      c.addChild(new _piTui.Text(fit(theme.fg("dim", `  ${itemTitle} ${rowNumber}: ${agentName}`)), 0, 0));
      c.addChild(new _piTui.Text(theme.fg("dim", `    status: pending`), 0, 0));
      c.addChild(new _piTui.Spacer(1));
      continue;
    }
    ...
    const resultOutput = (0, _utils.getSingleResultOutput)(r);
    ...
    theme.fg("warning", "warning") :
    theme.fg("success", "done");
    const stats = rProg ? ` | ${rProg.toolCount} tools, ${(0, _formatters.formatDuration)(rProg.durationMs)}` : "";
    ...
    const toolCallLines = getToolCallLines(r, expanded);
    c.addChild(new _piTui.Text(fit(stepHeader), 0, 0));

    const taskMaxLen = Math.max(20, w - 12);
    const taskPreview = expanded || r.task.length <= taskMaxLen ?
    r.task :
    `${r.task.slice(0, taskMaxLen)}...`;
    c.addChild(new _piTui.Text(fit(theme.fg("dim", `    task: ${taskPreview}`)), 0, 0));

    const outputTarget = extractOutputTarget(r.task);
    if (outputTarget) {
      c.addChild(new _piTui.Text(fit(theme.fg("dim", `    output: ${outputTarget}`)), 0, 0));
    }
    ...
    if (rRunning && rProg) {
      if (rProg.skills?.length) {
        c.addChild(new _piTui.Text(fit(theme.fg("accent", `    skills: ${rProg.skills.join(", ")}`)), 0, 0));
      }
  ...
  if (d.artifacts) {
    c.addChild(new _piTui.Spacer(1));
    c.addChild(new _piTui.Text(fit(theme.fg("dim", `Artifacts dir: ${(0, _formatters.shortenPath)(d.artifacts.dir)}`)), 0, 0));
  }
  return c;
} /* v9-9944f2a431b0090a */
