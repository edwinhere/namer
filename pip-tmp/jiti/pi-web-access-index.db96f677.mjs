...
var _piAi = await jitiImport("@mariozechner/pi-ai");
var _extract = await jitiImport("./extract.js");
var _githubExtract = await jitiImport("./github-extract.js");
var _geminiSearch = await jitiImport("./gemini-search.js");
var _codeSearch = await jitiImport("./code-search.js");

var _utils = await jitiImport("./utils.js");
var _storage = await jitiImport("./storage.js");
...
var _curatorServer = await jitiImport("./curator-server.js");
var _summaryReview = await jitiImport("./summary-review.js");
...
var _geminiWebConfig = await jitiImport("./gemini-web-config.ts");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}

const WEB_SEARCH_CONFIG_PATH = (0, _nodePath.join)((0, _nodeOs.homedir)(), ".pi", "web-search.json");
...
function loadConfig() {
  if (!(0, _nodeFs.existsSync)(WEB_SEARCH_CONFIG_PATH)) return {};
  const raw = (0, _nodeFs.readFileSync)(WEB_SEARCH_CONFIG_PATH, "utf-8");
  try {
...
function saveConfig(updates) {
  let config = {};
  if ((0, _nodeFs.existsSync)(WEB_SEARCH_CONFIG_PATH)) {
    const raw = (0, _nodeFs.readFileSync)(WEB_SEARCH_CONFIG_PATH, "utf-8");
    try {
  ...
  (0, _nodeFs.writeFileSync)(WEB_SEARCH_CONFIG_PATH, JSON.stringify(config, null, 2) + "\n");
}
...
async function getProviderAvailability() {
  const geminiWebAvail = await (0, _geminiWeb.isGeminiWebAvailable)();
  return {
    perplexity: (0, _perplexity.isPerplexityAvailable)(),
    exa: (0, _exa.isExaAvailable)(),
    gemini: (0, _geminiApi.isGeminiApiAvailable)() || !!geminiWebAvail
  };
}

async function loadCuratorBootstrap(requestedProvider) {
...
let pendingCurate = null;

function cancelPendingCurate(reason = "stale") {
...
function findGlimpseMjs() {
  try {
    const req = (0, _nodeModule.createRequire)("file:///home/edwin/.config/nvm/versions/node/v22.20.0/lib/node_modules/pi-web-access/index.ts");
    return req.resolve("glimpseui");
  } catch {

    // Optional dependency.
  }try {
    const globalRoot = (0, _nodeChild_process.execFileSync)("npm", ["root", "-g"], { encoding: "utf-8" }).trim();
    const entry = (0, _nodePath.join)(globalRoot, "glimpseui", "src", "glimpse.mjs");
    if ((0, _nodeFs.existsSync)(entry)) return entry;
  } catch {
...
async function getGlimpseOpen() {
  ...
  if (resolved) {
    try {
      glimpseOpen = (await ((specifier) => new Promise((r) => r(`${specifier}`)).then((s) => jitiImport(s).then((m) => _interopRequireWildcard(m))))(resolved)).open;
      return glimpseOpen;
    } catch {}
  }
  glimpseOpen = null;
...
function updateWidget(ctx) {
  ...
  const entries = _activity.activityMonitor.getEntries();
  const lines = [];
  ...
  const rateInfo = _activity.activityMonitor.getRateLimitInfo();
  ...
  ctx.ui.setWidget("web-activity", new _piTui.Text(lines.join("\n"), 0, 0));
}
...
theme)
{
  ...
  const target =
  entry.type === "api" ?
  `"${(0, _piTui.truncateToWidth)(entry.query || "", 28, "")}"` :
  (0, _piTui.truncateToWidth)(entry.url?.replace(/^https?:\/\//, "") || "", 30, "");
...
function handleSessionChange(ctx) {
  abortPendingFetches();
  closeCurator();
  (0, _githubExtract.clearCloneCache)();
  sessionActive = true;
  (0, _storage.restoreFromSession)(ctx);
  // Unsubscribe before clear() to avoid callback with stale ctx
  widgetUnsubscribe?.();
  widgetUnsubscribe = null;
  _activity.activityMonitor.clear();
  if (widgetVisible) {
    // Re-subscribe with new ctx
    widgetUnsubscribe = _activity.activityMonitor.onUpdate(() => updateWidget(ctx));
    updateWidget(ctx);
  }
}
...
function _default(pi) {
  ...
  function startBackgroundFetch(urls) {
    ...
    const fetchId = (0, _storage.generateId)();
    const controller = new AbortController();
    pendingFetches.set(fetchId, controller);
    (0, _extract.fetchAllContent)(urls, controller.signal).
    then((fetched) => {
      ...
      (0, _storage.storeResult)(fetchId, data);
  ...
  function storeAndPublishSearch(results) {
    const id = (0, _storage.generateId)();
    const data = {
      id, type: "search", timestamp: Date.now(), queries: results
    };
    (0, _storage.storeResult)(id, data);
    pi.appendEntry("web-search-results", data);
    return id;
  }
  ...
  function buildCurationCancelledReturn(reason) {
  ...
  candidates)
  {
    for (const { provider, id } of candidates) {
      const model = (0, _piAi.getModel)(provider, id);
      if (!model) continue;
  ...
  async function rewriteSearchQuery(query, ctx, signal) {
    ...
    const response = await (0, _piAi.complete)(
      model,
      {
        messages: [{
          role: "user",
  ...
  feedback)
  {
    ...
    try {
      return await (0, _summaryReview.generateSummaryDraft)(selectedResults, summaryContext, signal, modelOverride, feedback);
    } catch (err) {
      ...
      const deterministic = (0, _summaryReview.buildDeterministicSummary)(selectedResults);
      return {
  ...
  resultsByIndex)
  {
    ...
    const deterministic = (0, _summaryReview.buildDeterministicSummary)(fallbackResults);
    return {
  ...
  async function openCuratorBrowser(pc, searchesComplete = true) {
    let handle = null;
    try {
      pc.phase = "curating";
      ...
      const sessionToken = (0, _nodeCrypto.randomUUID)();
      handle = await (0, _curatorServer.startCuratorServer)(
        {
        ...
        },
        {
          ...
          async onAddSearch(query, queryIndex, provider) {
            ...
            normalizedProvider;
            try {
              const { answer, results, inlineContent, provider: actualProvider } = await (0, _geminiSearch.search)(query, {
      ...
      const open = (0, _nodeOs.platform)() === "darwin" ? await getGlimpseOpen() : null;
      if (open) {
        try {
  ...
  pi.registerTool({
    ...
    parameters: _typebox.Type.Object({
      query: _typebox.Type.Optional(_typebox.Type.String({ description: "Single search query. For research tasks, prefer 'queries' with multiple varied angles instead." })),
      queries: _typebox.Type.Optional(_typebox.Type.Array(_typebox.Type.String(), { description: "Multiple queries searched in sequence, each returning its own synthesized answer. Prefer this for research — vary phrasing, scope, and angle across 2-4 queries to maximize coverage. Good: ['React vs Vue performance benchmarks 2026', 'React vs Vue developer experience comparison', 'React ecosystem size vs Vue ecosystem']. Bad: ['React vs Vue', 'React vs Vue comparison', 'React vs Vue review'] (too similar, redundant results)." })),
      numResults: _typebox.Type.Optional(_typebox.Type.Number({ description: "Results per query (default: 5, max: 20)" })),
      includeContent: _typebox.Type.Optional(_typebox.Type.Boolean({ description: "Fetch full page content (async)" })),
      recencyFilter: _typebox.Type.Optional(
        (0, _piAi.StringEnum)(["day", "week", "month", "year"], { description: "Filter by recency" })
      ),
      domainFilter: _typebox.Type.Optional(_typebox.Type.Array(_typebox.Type.String(), { description: "Limit to domains (prefix with - to exclude)" })),
      provider: _typebox.Type.Optional(
        (0, _piAi.StringEnum)(["auto", "perplexity", "gemini", "exa"], { description: "Search provider (default: auto)" })
      ),
      workflow: _typebox.Type.Optional(
        (0, _piAi.StringEnum)(["none", "summary-review"], {
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (shouldCurate) {
        ...
        for (let qi = 0; qi < queryList.length; qi++) {
          ...
          try {
            const { answer, results, inlineContent, provider } = await (0, _geminiSearch.search)(queryList[qi], {
              provider: requestedProvider,
              numResults: params.numResults,
              recencyFilter: params.recencyFilter,
              domainFilter: params.domainFilter,
      ...
      for (let i = 0; i < queryList.length; i++) {
        ...
        try {
          const { answer, results, inlineContent, provider } = await (0, _geminiSearch.search)(query, {
    ...
    renderCall(args, theme) {
      ...
      const queryList = normalizeQueryList(rawQueryList);
      if (queryList.length === 0) {
        return new _piTui.Text(theme.fg("toolTitle", theme.bold("search ")) + theme.fg("error", "(no query)"), 0, 0);
      }
      if (queryList.length === 1) {
        const q = queryList[0];
        const display = q.length > 60 ? q.slice(0, 57) + "..." : q;
        return new _piTui.Text(theme.fg("toolTitle", theme.bold("search ")) + theme.fg("accent", `"${display}"`), 0, 0);
      }
      ...
      return new _piTui.Text(lines.join("\n"), 0, 0);
    },
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      if (isPartial) {
        if (details?.phase === "curating") {
          return new _piTui.Text(theme.fg("accent", "waiting for summary approval..."), 0, 0);
        }
        if (details?.phase === "searching") {
          ...
          return new _piTui.Text(theme.fg("accent", `[${bar}] ${display}`), 0, 0);
        }
        const progress = details?.progress ?? 0;
        const bar = "\u2588".repeat(Math.floor(progress * 10)) + "\u2591".repeat(10 - Math.floor(progress * 10));
        return new _piTui.Text(theme.fg("accent", `[${bar}] ${details?.phase || "searching"}`), 0, 0);
      }

      if (details?.error) {
        return new _piTui.Text(theme.fg("error", `Error: ${details.error}`), 0, 0);
      }
      ...
      if (!expanded) {
        const box = new _piTui.Box(1, 0, (t) => theme.bg("toolSuccessBg", t));
        box.addChild(new _piTui.Text(statusLine, 0, 0));
        ...
        if (summaryPreview) {
          ...
          box.addChild(new _piTui.Text(theme.fg("dim", preview), 0, 0));
          collapsedLines++;
          ...
          for (const cq of details.curatedQueries.slice(0, 3)) {
            ...
            box.addChild(new _piTui.Text(theme.fg("accent", `  "${dq}"`) + suffix, 0, 0));
            collapsedLines++;
          }
          if (details.curatedQueries.length > 3) {
            box.addChild(new _piTui.Text(theme.fg("dim", `  ... and ${details.curatedQueries.length - 3} more`), 0, 0));
            collapsedLines++;
          }
        } else {
          const textContent = result.content.find((c) => c.type === "text")?.text || "";
  ...
  pi.registerTool({
    ...
    parameters: _typebox.Type.Object({
      query: _typebox.Type.String({ description: "Programming question, API, library, or debugging topic to search for" }),
      maxTokens: _typebox.Type.Optional(_typebox.Type.Integer({
        minimum: 1000,
        maximum: 50000,
    ...
    async execute(toolCallId, params, signal) {
      return (0, _codeSearch.executeCodeSearch)(toolCallId, params, signal);
    },
    ...
    renderCall(args, theme) {
      ...
      return new _piTui.Text(theme.fg("toolTitle", theme.bold("code_search ")) + theme.fg("accent", display), 0, 0);
    },
    ...
    renderResult(result, { expanded }, theme) {
      ...
      if (!expanded) return new _piTui.Text(summary, 0, 0);

      const textContent = result.content.find((c) => c.type === "text")?.text || "";
      const preview = textContent.length > 500 ? textContent.slice(0, 500) + "..." : textContent;
      return new _piTui.Text(summary + "\n" + theme.fg("dim", preview), 0, 0);
    }
  });
  ...
  pi.registerTool({
    ...
    parameters: _typebox.Type.Object({
      url: _typebox.Type.Optional(_typebox.Type.String({ description: "Single URL to fetch" })),
      urls: _typebox.Type.Optional(_typebox.Type.Array(_typebox.Type.String(), { description: "Multiple URLs (parallel)" })),
      forceClone: _typebox.Type.Optional(_typebox.Type.Boolean({
      ...
      timestamp: _typebox.Type.Optional(_typebox.Type.String({
      ...
      frames: _typebox.Type.Optional(_typebox.Type.Integer({
        minimum: 1,
        maximum: 12,
    ...
    async execute(_toolCallId, params, signal, onUpdate) {
      ...
      const fetchResults = await (0, _extract.fetchAllContent)(urlList, signal, {
      ...
      const responseId = (0, _storage.generateId)();
      const data = {
        id: responseId,
        type: "fetch",
        timestamp: Date.now(),
      ...
      (0, _storage.storeResult)(responseId, data);
      pi.appendEntry("web-search-results", data);
    ...
    renderCall(args, theme) {
      ...
      if (urlList.length === 0) {
        return new _piTui.Text(theme.fg("toolTitle", theme.bold("fetch ")) + theme.fg("error", "(no URL)"), 0, 0);
      }
      const lines = [];
      if (urlList.length === 1) {
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      if (isPartial) {
        const progress = details?.progress ?? 0;
        const bar = "\u2588".repeat(Math.floor(progress * 10)) + "\u2591".repeat(10 - Math.floor(progress * 10));
        return new _piTui.Text(theme.fg("accent", `[${bar}] ${details?.phase || "fetching"}`), 0, 0);
      }
      ...
      if (details?.urlCount === 1) {
        ...
        if (typeof details?.duration === "number") {
          statusLine += theme.fg("muted", ` | ${(0, _utils.formatSeconds)(Math.floor(details.duration))} total`);
        }
        const textContent = result.content.find((c) => c.type === "text")?.text || "";
        if (!expanded) {
          const brief = textContent.length > 200 ? textContent.slice(0, 200) + "..." : textContent;
          return new _piTui.Text(statusLine + "\n" + theme.fg("dim", brief), 0, 0);
        }
        const lines = [statusLine];
        if (details?.prompt) {
      ...
      if (!expanded) {
        return new _piTui.Text(statusLine, 0, 0);
      }
      const textContent = result.content.find((c) => c.type === "text")?.text || "";
      const preview = textContent.length > 500 ? textContent.slice(0, 500) + "..." : textContent;
      return new _piTui.Text(statusLine + "\n" + theme.fg("dim", preview), 0, 0);
  ...
  pi.registerTool({
    ...
    parameters: _typebox.Type.Object({
      responseId: _typebox.Type.String({ description: "The responseId from web_search or fetch_content" }),
      query: _typebox.Type.Optional(_typebox.Type.String({ description: "Get content for this query (web_search)" })),
      queryIndex: _typebox.Type.Optional(_typebox.Type.Number({ description: "Get content for query at index" })),
      url: _typebox.Type.Optional(_typebox.Type.String({ description: "Get content for this URL" })),
      urlIndex: _typebox.Type.Optional(_typebox.Type.Number({ description: "Get content for URL at index" }))
    }),

    async execute(_toolCallId, params) {
      const data = (0, _storage.getResult)(params.responseId);
      if (!data) {
        return {
    ...
    renderCall(args, theme) {
      ...
      let target = "";
      if (query) target = `query="${query}"`;else
      if (queryIndex !== undefined) target = `queryIndex=${queryIndex}`;else
      if (url) target = url.length > 30 ? url.slice(0, 27) + "..." : url;else
      if (urlIndex !== undefined) target = `urlIndex=${urlIndex}`;
      return new _piTui.Text(theme.fg("toolTitle", theme.bold("get_content ")) + theme.fg("accent", target || responseId.slice(0, 8)), 0, 0);
    },
  ...
  pi.registerCommand("websearch", {
    ...
    handler: async (args, ctx) => {
      ...
      try {
        const handle = await (0, _curatorServer.startCuratorServer)(
          {
            queries,
            sessionToken,
          ...
          },
          {
            ...
            async onAddSearch(query, queryIndex, provider) {
              ...
              normalizedProvider;
              try {
                const { answer, results, provider: actualProvider } = await (0, _geminiSearch.search)(query, {
                  provider: requestedProvider,
        ...
        if (queries.length > 0) {
          (async () => {
            for (let qi = 0; qi < queries.length; qi++) {
              ...
              try {
                const { answer, results, provider } = await (0, _geminiSearch.search)(queries[qi], {
                  provider: requestedProvider,
                  signal: searchAbort.signal
                });
                if (aborted || activeCurator !== handle) break;
  ...
  pi.registerCommand("google-account", {
    ...
    handler: async () => {
      if (!(0, _geminiWebConfig.isBrowserCookieAccessAllowed)()) {
        pi.sendMessage({
      ...
      const email = await (0, _geminiWeb.getActiveGoogleEmail)(cookies);
      const text = email ?
  ...
  pi.registerCommand("search", {
    ...
    handler: async (_args, ctx) => {
      const results = (0, _storage.getAllResults)();
      ...
      if (action === "Delete") {
        (0, _storage.deleteResult)(selected.id);
